...
plot_data   <- lapply(c("Asian American", "Latino"),
...
p <- ggplot() +
  ...
  geom_rect(
    data = bar_data[x == 1],
    aes(xmin = X_LEFT - BAR_W, xmax = X_LEFT,
  ...
  geom_rect(
    data = bar_data[x == 2],
    aes(xmin = X_RIGHT, xmax = X_RIGHT + BAR_W,
        ymin = ymin, ymax = ymax, fill = direction),
    color = "white", linewidth = 0.25
  ) +
  ...
  annotate("text", x = X_LEFT  - BAR_W / 2, y = -0.05,
           label = "1960s–1970s", size = 4.2, fontface = "bold", hjust = 0.5) +
  annotate("text", x = X_RIGHT + BAR_W / 2, y = -0.05,
  ...
  labs(
    ...
    subtitle = "Periods defined by IRS incorporation year",
    x        = NULL
  ) +
